        ...
        public override void Play()
        {
                ...
                foreach (Player player in Players)
                {
                    ...
                    if (i == 1)
                    {
                        ...
                        if (blackJack)
                        {
                            ...
                            Console.WriteLine("Blackjack! {0} wins {1}. {0}'s balance is now: {2}.", player.Name, Bets[player], player.Balance);
                            return;
                        }
                    }
                }
            ...
            if (Dealer.isBusted)
            {
                ...
                foreach (KeyValuePair<Player, int> entry in Bets)
                {
                    ...
                    Console.WriteLine("{0} won {1}! {0}'s balance is now: {2}.", entry.Key.Name, entry.Value, entry.Key.Balance);
                    Dealer.Balance -= entry.Value;
                }
                return;
            }
